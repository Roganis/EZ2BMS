...
| A game chart exported for the cabinet: every note's track, tick, time, sound, velocity, pan, kind and raw length; the kept records; the tempo map; the header                                                        | `publish/chart-plan.ts` (`target: 'cabinet'`) | `cabinet.oracle.test.ts` - the synthetic game's charts (v8 and v6) and 60 random game charts, imported, compiled for the cabinet and read back by the port's parser, equal to the originals                                                                                                                                                             |
...
- **The original cabinet.** The original executable has one voice per
  ...
  note. The promise is for EZ2PORT; a cabinet export lints it
  (`cabinet-voice-lane`, `exporting.md`).
...
## Exporting to the original game (M6, the executable not in the oracle)

The oracle is EZ2PORT's core, not the original executable, so what the
export writes is proven readable by the port's own readers (the rows above),
and where the original may differ is linted from what is written down of
it:

- **131068 bytes per file.** The original decrypts a chart, `.ezi` or `.ini`
  into one stack frame of `header[128] data[131072] out[131068]`
  (`third_party/ez2port-core/ez2/crypt.c`); a bigger file cannot load
  (`cabinet-size`).
- **One sound per track.** Its sound entries are indexed by the chart's
  track (`reference/play.c`), so a new sound on a track cuts the last,
  keyed or not (`cabinet-voice-backing`, `cabinet-voice-lane`).
- **2047 keysounds** (`ez2/ezi.h`: a 0x800-entry table, slot 0 unused), every
  slice counted (`cabinet-slots`).

Where an export deliberately differs from the game's own files:

| What                  | The game's chart                                  | Exported                                                                                           |
| --------------------- | ------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| Tempo records         | on any track, duplicates at one tick              | on track 0, one per change: the tempo map, which is equal, is what is promised                     |
| `.ezi` slots          | the author's numbering; two slots may name a file | numbered afresh; two slots naming one file are one (the import made them one voice)                |
| New background sounds | -                                                 | placed around the game's own, on free tracks, never over one; tracks added rather than a sound cut |
| Keysound files        | -                                                 | never written over: equal audio reuses the file, anything else is `name~n`                         |
| `.ini`                | as shipped                                        | written only when its values change, as `Key=value` with CRLF                                      |
| The end of the stage  | the last record                                   | an imported chart keeps its `total_ticks` and gets no closing record; a new chart gets publish's   |
| `song.bin` `a`        | a per-tier float whose meaning is not known       | left alone; a new tier takes its siblings' when they agree, else 0                                 |
| A missing sound       | a dangling name that plays nothing                | the same: its `.ezi` line kept, no file written                                                    |

What the owner's cabinet must confirm is listed in `AI-DISCLOSURE.md`.

## Exporting BMS (M6, not read by EZ2PORT)

EZ2PORT reads no BMS. The writer is checked by EZ2BMS's own BMS reader,
which `bms.test.ts` checks against the BMS command memo's arithmetic:
random charts written, read and converted back give every note in its lane,
